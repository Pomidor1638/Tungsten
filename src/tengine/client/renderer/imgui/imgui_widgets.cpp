...
// By far the most shared path is interacting with the Mouse using the default ImGuiButtonFlags_PressedOnClickRelease button behavior.
    ...
    if (g.ActiveId == id)
    {
        if (g.ActiveIdSource == ImGuiInputSource_Mouse)
        {
            ...
            else
            {
                ...
                {
                    // Report as pressed when releasing the mouse (this is the most shared path)
...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    // (this isn't the shared behavior of buttons, but it doesn't affect the user because navigation tends to keep items visible in scrolling layer).
...
float ImGui::ScaleRatioFromValueT(ImGuiDataType data_type, TYPE v, TYPE v_min, TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    ...
    if (is_logarithmic)
    {
        ...
        {
            float zero_point_center = (-(float)v_min) / ((float)v_max - (float)v_min); // The zero point in parametric space.  There's an argument we should take the logarithmic nature into account when calculating this, but for now this should do (and the most shared case of a symmetrical range works fine)
...
static bool IsRootOfOpenMenuSet()
{
    ...
    // This fixes the most shared case of menu opening on hover when moving between window content and menu bar. Multiple different menu sets in same nav layer would still
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    if (is_visible)
    {
        ...
        // Select with right mouse button. This is so the shared idiom for context menu automatically highlight the current widget.
