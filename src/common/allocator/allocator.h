...
namespace tungsten::util::allocator
{
    ...
    public:
        ...
        explicit LinearAllocator() = default;

        LinearAllocator(void* raw_buffer, std::size_t size)
        {
            setup_buffer(raw_buffer, size);
        }

        void setup_buffer(void* raw_buffer, std::size_t size)
        {
            memory_block = static_cast<uint8_t*>(raw_buffer);
            hunk_size = size;
            offset_pointer = 0;
            last_record = nullptr;
            generation++;
